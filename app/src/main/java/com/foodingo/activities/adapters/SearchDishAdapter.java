...
public class SearchDishAdapter extends ArrayAdapter<Dish> implements Filterable {
	...
	public SearchDishAdapter(Context context, List<Dish> objects) {
		super(context, layout.row_search_dish, objects);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			...
			convertView = mLayoutInflater.inflate(layout.row_dish_adapter, null);
		}
		if (mdishes.size() > 0 && position < mdishes.size()) {
			...
			dishImage.setPlaceholder(convertView.getResources().getDrawable(
					drawable.placeholder_icon));
			Glide.with(convertView.getContext()).load(d.getImageUrl())
					.into(dishImage);
